...
toto=ip.pct_change(12)*100
data = pandas.concat([toto[:-20],business_climate_industry[:-20]],axis=1)
model = pandas.ols(y=data['Industrial production index'],x=data['Business climate - industry'])
#prediction = model.predict(data['Business climate - industry'])
